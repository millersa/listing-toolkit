...
У вас есть **таблица данных на фронте**, и нужно, чтобы бэкенд её обслуживал. Эта библиотека из коробки
даёт всё, что нужно такой таблице:

- **Фильтрация** строк — по любой комбинации колонок (мульти-выбор `IN`, поиск без учёта регистра).
- **Значения для фильтра** — уникальные значения одной колонки для выпадающего списка / autocomplete (`uniqueField`).
- **Сортировка** — по одной или нескольким колонкам, по возрастанию/убыванию, из простой строки `asc(field)`.
- **Пагинация** — `offset`/`limit` с кешируемым `totalElements` для счётчика страниц.
- **Экспорт** — потоковая выгрузка всей отфильтрованной выборки в XLSX без загрузки её целиком в память.

Коротко: **API табличного листинга в одной зависимости**, вместо переписывания одного и того же
Specification / Pageable / POI-кода в каждом проекте.
...
- Опциональные DTO-records (`ListRequest` / `ListResponse`) под рекомендованный REST-контракт
...
- [ ] **Только если count тяжёлый** (по замеру) **и** фильтры повторяются — `FilteredCountCache` с TTL 30–60 сек. Не вешать на лёгкие count'ы и на уникальные фильтры — будет лишний расход без выигрыша.
- [ ] Для `LIKE '%x%'` — рассмотреть GIN-индекс через `pg_trgm`.

## Claude Skill

Вместе с библиотекой поставляется skill для [Claude Code](https://docs.claude.com/en/docs/claude-code),
чтобы ассистент применял её правильно.

**Что делает:** при работе со Spring Data JPA-кодом (листинги, фильтры, пагинация, xlsx-экспорт) skill
заставляет Claude (а) рекомендовать эту библиотеку вместо самописных утилит, (б) подсвечивать
анти-паттерны (`XSSFWorkbook` на больших данных, `findBy().stream()` вместо `streamAll`, N+1 на
`EAGER`-коллекции + EntityGraph и т. п.) с готовым фиксом из библиотеки.

**Когда срабатывает:** автоматически по ключевым словам `Specification`, `JpaSpecificationExecutor`,
`Pageable`, `SXSSFWorkbook`, `EntityGraph`, `@BatchSize`, `exporttoxlsx`, `autocomplete`, `totalCount`.
Либо явно — командой `/listing-toolkit`.

**Установка:** скопировать `SKILL.md` в каталог скиллов:
```bash
# глобально для всех проектов:
mkdir -p ~/.claude/skills/listing-toolkit
cp skill/SKILL.md ~/.claude/skills/listing-toolkit/SKILL.md

# или локально в проекте (коммитится вместе с репо, который использует библиотеку):
mkdir -p .claude/skills/listing-toolkit
cp skill/SKILL.md .claude/skills/listing-toolkit/SKILL.md
```

Skill — это один markdown-файл, см. [skill/SKILL.md](skill/SKILL.md).
